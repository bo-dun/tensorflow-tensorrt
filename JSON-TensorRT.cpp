...
class Profiler : public IProfiler {
    ...
    void reportLayerTime(const char* layerName, float ms) override {
        cout << string(layerName) << ": " << ms << " milliseconds" << endl; 
    }
...
ITensor* createConvolutional(INetworkDefinition* network, ITensor& input, Json::Value& layer, set<string>& layer_names, map<string, Weights>& weight_map) {
    ...
    cv->setPadding(DimsHW{(filter_height - 1)/2, (filter_width - 1)/2});
...
void doInference(IExecutionContext& context, vector<float*>& input, vector<float*>& output, int batchSize) {
    ...
    for (unsigned int i = 0; i < input.size(); i++) {
        ...
        cout << "Time taken (in microseconds) : " << end.tv_usec - start.tv_usec << endl;
    }
